...
{
    public StacsEntity ownship;
    public StacsEntity target;
    ...
    public Potential(StacsEntity own, StacsEntity tgt)
    {
        ownship = own;
        target = tgt;
...
public class CPAInfo
{
    ...
    public CPAInfo(StacsEntity e1, StacsEntity e2)
    {
        ownship = e1;
        target = e2;
    }
...
public class DistanceMgr : MonoBehaviour
{
    ...
    public Dictionary<StacsEntity, Dictionary<StacsEntity, Potential>> potentialsDictionary;
    ...
    {
        isInitialized = true;
        potentialsDictionary = new Dictionary<StacsEntity, Dictionary<StacsEntity, Potential>>();
        potentialsList = new List<List<Potential>>();
        int n = EntityMgr.inst.entities.Count;
        potentials2D = new Potential[n, n];
        i = 0;
        foreach (StacsEntity ent1 in EntityMgr.inst.entities) {
            Dictionary<StacsEntity, Potential> ent1PotDictionary = new Dictionary<StacsEntity, Potential>();
            ...
            j = 0;
            foreach (StacsEntity ent2 in EntityMgr.inst.entities) {
    ...
    public Potential GetPotential(StacsEntity e1, StacsEntity e2)
    {
        Potential p = null;
        if (isInitialized)
